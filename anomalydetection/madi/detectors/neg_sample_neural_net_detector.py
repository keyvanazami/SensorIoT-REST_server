...
class NegativeSamplingDataset:
  ...
  def __call__(self):
    ...
    x = np.float32(np.asarray(training_sample[self._column_order]))
...
class NegativeSamplingNeuralNetworkAD(
  ...
  def train_model(self, x_train: pd.DataFrame) -> None:
    ...
    early_stopping = tf.keras.callbacks.EarlyStopping(
        monitor='val_binary_accuracy', patience=self._patience, mode='max', restore_best_weights=True)
  ...
  def predict(self, sample_df: pd.DataFrame) -> pd.DataFrame:
    ...
    sample_df['class_prob'] = y_hat.flatten()
    return sample_df
